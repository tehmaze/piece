...
static unsigned char artworx_palette_slots[ARTWORX_PALETTE_DST_COLORS] = {
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x14, 0x07,
    0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
};

screen *artworx_parser_read(FILE *fd, const char *filename)
{
    screen *display = NULL;
    sauce *record = NULL;
    int x = 0;
    ...
    display = screen_create(80, 1, record);
    if (display == NULL) {
    ...
    artworx_palette = palette_new("from file", 0);
    rgb_color rgb;
    for (uint8_t i = 0; i < ARTWORX_PALETTE_SRC_COLORS; ++i) {
        ...
        ch = fgetc(fd);
        rgb.b = (ch << 2) | (ch >> 4);
        palette_add_color(artworx_palette, &rgb);
    }
    ...
    for (uint8_t i = 0; i < ARTWORX_PALETTE_DST_COLORS; ++i) {
        palette_add_color(
            display->palette,
            &artworx_palette->color[artworx_palette_slots[i]]
        );
    }
...
void artworx_parser_init(void) {
    parser_register(&artworx_parser);
}
