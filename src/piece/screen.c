                               ...
                               piece_screen_tile *initial)
{
    ...
    display->initial = piece_allocate(sizeof(piece_screen_tile));

    /* Initial colors and attributes for new tiles */
    if (initial == NULL) {
        display->initial->attrib = PIECE_ATTRIB_DEFAULT;
        display->initial->fg = PIECE_TILE_DEFAULT_FG;
        display->initial->bg = PIECE_TILE_DEFAULT_BG;
        display->initial->ch = ' ';
    } else {
        memcpy(display->initial, initial, sizeof(piece_screen_tile));
    }
    ...
    if (record != NULL && ((
        ...
        for (int32_t i = 0; i < display->tiles; ++i) {
            piece_screen_tile_reset(display, tile++);
        }
    }
...
void piece_screen_insert_line(piece_screen *display, int32_t y)
{
    ...
    for (i = 0; i < display->size.width; ++i) {
        piece_screen_tile_reset(display, &display->tile[offset + i]);
    }
}
...
{
    for (int32_t i = 0; i < display->tiles; ++i) {
        piece_screen_tile_reset(display, &display->tile[i]);
    }
}

bool piece_screen_tile_reset(piece_screen *display, piece_screen_tile *tile) {
    memcpy(tile, display->initial, sizeof(piece_screen_tile));
    return true;
}
...
piece_screen_tile *screen_tile_append(piece_screen *display)
{
    ...
    memcpy(current, display->initial, sizeof(piece_screen_tile));
